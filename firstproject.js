...
var question3=new questions("Identify the correct form of the verb to complete the sentence: She ___ to the store.",["Go", "Goes","Going","Gone"],"Goes",3);
...
var question5=new questions("Choose the correct form of the verb to complete the sentence:They ___ swimming every weekend.",["Go","Goes","Going","Go"],"Go",5);
...
function checkedHandle(){
    if(savedAnswersArr[selectedQuestions[counter]].answer!==""){
        console.log(savedAnswersArr[selectedQuestions[counter]].answer);
...
function generateQuestions(){
    questionArray.forEach(function(el,index) {
        if(el.id===selectedQuestions[counter]){
            questionh.textContent=el.title; 
...
nextbutton.addEventListener("click",function(){
    ...
    previousButton.addEventListener("click",function(){
            ...
            questionArray.forEach(function(el,index) {
                if(el.id===selectedQuestions[counter]){
                    ...
                    console.log(selectedQuestions[counter]);            /////////
            });
        qNumbersDiv.textContent=counter+1;
        if(counter===0){
    ...
    markbutton.addEventListener("click",function(){
        if(markedQuestions.indexOf(counter)===-1){
            markListDiv.innerHTML+=`<div>mrk-Question${counter+1}</div>`;
...
console.log(document.querySelectorAll("#exam label")[x].textContent);
savedAnswersArr[counter].answer=document.querySelectorAll("#exam label")[x].textContent;
}
...
for(var i=0;i<10;i++){
    if(savedAnswersArr[i].answer===questionArray[selectedQuestions[i]].rightAnswer){
grade++;
    }
}
